...
function draw_cal_new($id_item,$month,$year,$manage_type="avail"){
	global $lang;
	
	$the_cal='
	<div id="'.$month.'_'.$year.'" class="cal_title">'.$lang["month_".$month.""].' '.$year.'</div>';
	
	$list_days='<li class="aparname_head"><div id="cal_in">Property Name</div></li>';
	$list_days.='<li class="nowidth_head"><div id="cal_in">'.date("M",mktime(0, 0, 0, $month, 1, $year)).'</div></li>';
	...
	for($day_counter = 1; $day_counter <= $days_in_this_month; $day_counter++){
		...
		$list_days .= '
		<li class=" modify_head "  id="date_'.$date_db.'" ><div id="cal_in">'.$day_counter.'</div></li>';
	...
	{
	$sql_condition="AND b.id_user='".$_SESSION['admin_id']."'";
	}
	...
	$res_new=mysql_query($sql) or die("Error checking items<br>".mysql_Error());
	
	while($rowname=mysql_fetch_array($res_new)){
		//	create an array of items to be able to confirm that the calendar 
		$id_item=$rowname["id"];
		$small_name = substr($rowname["the_item"], 0, 15);
	$list_days='<li class="aparname" title="'.$rowname["the_item"].'"><div id="cal_in">'.$small_name.'</div></li>';
	$list_days.='<li class="nowidth">&nbsp;</li>';
	$month=sprintf("%02s",$month);
	//	define vars
	$today_timestamp	=   mktime(0,0,0,date('m'),date('d'),date('Y'));	# 	current timestamp - used to check if date is in past
	$this_month 		= 	getDate(mktime(0, 0, 0, $month, 1, $year));		# 	convert month to timestamp
	...
	for($day_counter = 1; $day_counter <= $days_in_this_month; $day_counter++){
		...
		if(array_key_exists($date_db,$booked_days)){
			$day_classes.=" modify ".$booked_days[$date_db]["class"];
		...
		$list_days .= '
		<li class=" modify '.$day_classes.' "  id="date_'.$date_db.'__'.$id_item.'" title="'.$date_format.$day_title_state.'" data-date="'.$date_format.'">
		<div id="cal_in">';
                //GETTING PRICE FIELDS FOR PRICE SECTION IN CALENDAR         
                $user_id = $_SESSION['admin_id'];
                $apartment_id = $id_item;
                // $apartment_id = $_GET["id_item"];
				$userid = $_GET["id"];
				
					//ONLY TO VIEW EACH PROPERTY ON  - CREATE VIEW
					$items_sql="SELECT * FROM bookings_seasons WHERE aprt_id='".$apartment_id."' AND state = 1 ";
					...
					//ONLY FOR USERS
					if ($user_id > 1){
						$items_sql="SELECT * FROM bookings_seasons WHERE aprt_id='".$apartment_id."' AND user_id='".$user_id."' AND state = 1  "; 
					}
                ...
                $from = $items_row['date_from'];
                $to = $items_row['to_date'];
                    
                    if (($date_db >= $from) && ($date_db <= $to))
                    {                  
                      $list_days .= '<span id="price-color">R'.$items_row['price'].' </span>';
                    }
                    else
                    {
                    $list_days .= '';  
...
function draw_cal($id_item,$month,$year,$manage_type="avail"){
	...
	for($day_counter = 1; $day_counter <= $days_in_this_month; $day_counter++){
		...
		$list_days .= '
		<li class="'.$day_classes.' "  id="date_'.$date_db.'" title="'.$date_format.$day_title_state.'" data-date="'.$date_format.'">'.$day_counter.'<br/>';
                //GETTING PRICE FIELDS FOR PRICE SECTION IN CALENDAR         
                $user_id = $_SESSION['admin_id'];
                $apartment_id = $_GET["id_item"];
				$userid = $_GET["id"];
...
function sel_list_items($id_item_current){
	$list_items="";
	if($_GET['create_view'] == 1){
		$sql="SELECT id, desc_".AC_LANG." as the_item FROM ".T_BOOKINGS_ITEMS." WHERE id='".$_GET['id_item']."' state=1 ORDER BY list_order";
	}
	if($_SESSION ['admin_id'] == 1){
		$sql="SELECT id, desc_".AC_LANG." as the_item FROM ".T_BOOKINGS_ITEMS." WHERE state=1 ORDER BY list_order";
	}else{
		$sql="SELECT id, desc_".AC_LANG." as the_item FROM ".T_BOOKINGS_ITEMS." WHERE id_user='".$_SESSION['admin_id']."' AND state=1 ORDER BY list_order";
	}
...
function getLocalTimezone(){
	...
	//COMM THIS LINE OUT TO NOT DISPLAY ERRORS 05/11/2014
